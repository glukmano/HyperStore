<x-ui.card class="mt-6">
    <h2 class="text-xl font-bold mb-2">{{ __('Reviews') }}</h2>

    <p class="text-base-content/70 mb-4">
        @if($aggregate['count'] > 0)
            ⭐ {{ number_format($aggregate['average'], 1) }} ({{ $aggregate['count'] }} {{ __('reviews') }})
        @else
            {{ __('No reviews yet.') }}
        @endif
    </p>
    ...
    <div class="space-y-4 mb-6">
        ...
        @empty
            <x-ui.empty-state message="{{ __('Be the first to review this product.') }}" />
        @endforelse
    </div>
    ...
    @auth
        ...
        @else
            <p class="text-base-content/60 text-sm">{{ __('You have already reviewed this product.') }}</p>
        @endif
    @else
