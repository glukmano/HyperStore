<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-bold">{{ __('Review Moderation') }}</h1>
        <p class="text-sm text-base-content/60">{{ __('Approve or reject Product reviews awaiting moderation.') }}</p>
    </div>

    <x-ui.breadcrumbs :items="['Control Center' => route('control-center.dashboard'), 'Review Moderation' => null]" />
    ...
    <x-ui.card>
        <x-ui.table :headers="[__('Product'), __('Reviewer'), __('Rating'), __('Body'), __('Verified'), '']" :empty="$reviews->isEmpty()" emptyMessage="{{ __('No reviews in this status.') }}">
