...
`Modules\Messaging\Services\MessageAttachmentService`: MIME validated via `UploadedFile::getMimeType()` (real finfo content inspection, never client-declared type/extension), server-generated filenames, size-capped, stored on MediaLibrary's `local` (private) disk, served only via signed temporary URLs — never a predictable public path.
