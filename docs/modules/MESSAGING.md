...
`App\Livewire\Storefront\Account\{MessagesInbox,ConversationThread}` (`/account/messages[/{conversation}]`, auth-gated). `ConversationThread` is wired to the real Reverb transport: `#[On('echo-private:conversation.{conversation.id},.message.sent')]` on `onMessageBroadcast()` uses Livewire's built-in Laravel Echo integration (`js/features/supportLaravelEcho.js`) to subscribe the browser to the private `conversation.{id}` channel — the same channel/policy `routes/channels.php` already authorizes. The listener method body is intentionally empty: Livewire's own request/response cycle re-runs `render()`, which re-queries `messages` straight from PostgreSQL, so the socket payload itself is never trusted or rendered directly, a reconnect/duplicate delivery just re-triggers the same idempotent full re-render (deduplicated by `wire:key="msg-{id}"`), and an unauthorized user's Echo subscription never completes (channel auth rejects it before any event reaches the browser). `wire:poll.15s` remains only as a documented degradation fallback for a session where `window.Echo` never connected (e.g., no `VITE_REVERB_APP_KEY` at build time, or the Reverb server is down) — it is not the primary delivery path. Frontend bootstrap: `resources/js/echo.js` (imported by `resources/js/app.js`) constructs `window.Echo` from `laravel-echo`+`pusher-js` (both already present in `package.json`) using `VITE_REVERB_APP_KEY`/`VITE_REVERB_HOST`/`VITE_REVERB_PORT`/`VITE_REVERB_SCHEME`; when that key is empty, `window.Echo` is simply never created and the page still functions via the polling fallback. `Modules\Messaging\Livewire\MessagingModerationManager` (`/control-center/platform/messaging`, `messaging.moderate`) lists conversations tenant-wide and can close one — deliberately bypassing `ConversationPolicy`/`MessagingService::close()` (that path is participant self-service requiring a `conversation_participants` row) in favor of the `messaging.moderate` permission gate alone.

### Production Realtime Requirements

- A supervised `php artisan reverb:start` process (systemd/Supervisor — never a bare foreground process in production).
- A running queue worker (`php artisan queue:work`) — `MessageSent implements ShouldBroadcast` (not `ShouldBroadcastNow`), so Laravel's `BroadcastManager` queues the actual push to Reverb as a job; without a worker running, `DB::afterCommit()` still fires the dispatch, but the broadcast itself never leaves the app server.
- `REVERB_APP_ID`/`REVERB_APP_KEY`/`REVERB_APP_SECRET`/`REVERB_HOST`/`REVERB_PORT`/`REVERB_SCHEME` set on the server, and `BROADCAST_CONNECTION=reverb`.
- `VITE_REVERB_APP_KEY`/`VITE_REVERB_HOST`/`VITE_REVERB_PORT`/`VITE_REVERB_SCHEME` set at **build time** (`npm run build`) — these are compiled into the JS bundle, not read at request time, so a value change requires a rebuild.
- A reverse proxy that passes the `Upgrade`/`Connection` headers through to the Reverb port for WebSocket traffic (e.g., Nginx `proxy_set_header Upgrade $http_upgrade;`).
